...
function createStars() {
  ...
  for (let i = 0; i < CONFIG.starCount; i++) {
    ...
    stars.push({
      ...
      flash: 0,
      flashTimer: random(CONFIG.flashMinInterval, CONFIG.flashMaxInterval),
...
function drawStar(star: Star, time: number) {
  ...
  x += mouseX * CONFIG.parallaxStrength * (star.radius / Math.max(width, height));
  y += mouseY * CONFIG.parallaxStrength * (star.radius / Math.max(width, height));
...
function updateStars(delta: number) {
  for (let i = 0; i < stars.length; i++) {
    ...
    if (star.flash > 0) {
      ...
      if (star.flashTimer <= 0 && Math.random() < CONFIG.flashChance) {
        star.flash = 1;
      }
      if (star.flashTimer < -CONFIG.flashMaxInterval) {
        star.flashTimer = random(CONFIG.flashMinInterval, CONFIG.flashMaxInterval);
      }
    }
  }
}
...
function createMeteor() {
  ...
  meteors.push({
    ...
    length: isBig
      ...
      : random(CONFIG.meteorMinLength, CONFIG.meteMaxLength),
...
function drawMeteor(meteor: Meteor, delta: number) {
  ...
  if (meteor.isBig) {
    ...
    ctx.lineWidth = 12;
    ctx.shadowBlur = 22;
    ctx.shadowColor = isDark() ? `rgba(200,220,255,${opacity * 0.8})` : `rgba(130,140,225,${opacity * 0.5})`;
    ctx.beginPath();
    ...
    ctx.fillStyle = isDark() ? `rgba(255,250,240,${opacity})` : `rgba(160,170,240,${opacity})`;
    ctx.beginPath();
  ...
  ctx.shadowBlur = 14;
  ctx.shadowColor = isDark() ? `rgba(200,225,255,${opacity * 0.75})` : `rgba(90,115,210,${opacity * 0.6})`;
  ctx.beginPath();
  ctx.moveTo(meteor.x, meteor.y);
  ctx.lineTo(tailX, tailY);
  ctx.stroke();
  ...
  ctx.fillStyle = isDark() ? `rgba(255,255,255,${opacity})` : `rgba(90,115,210,${opacity})`;
  ctx.fill();
  ctx.restore();
}
...
function drawPlanet(planet: Planet, delta: number) {
  ...
  if (!planet.exploded) {
    ...
    const outOfView =
      planet.x < -200 || planet.x > width + 200 || planet.y < -200 || planet.y > height + 200;
  ...
  if (planet.exploded) {
    ...
    const ring = isDark() ? `rgba(255,200,150,${0.7 * fade})` : `rgba(200,140,90,${0.6 * fade})`;
    const ring2 = isDark() ? `rgba(255,230,200,${0.45 * fade})` : `rgba(220,170,120,${0.4 * fade})`;
    ctx.save();
    ctx.globalCompositeOperation = "lighter";
    ctx.lineWidth = 2;
    ctx.shadowBlur = 30;
...
function updatePlanets(delta: number) {
  if (!reduceMotion && planets.length === 0 && performance.now() >= nextPlanetTime) {
    createPlanet();
    nextPlanetTime = performance.now() + random(CONFIG.planetMinInterval, CONFIG.planetMaxInterval);
  }
...
function updateMeteors(delta: number) {
  if (!reduceMotion && meteors.length < CONFIG.maxSimultaneousMeteors && performance.now() >= nextMeteorTime) {
    createMeteor();
    scheduleNextMeteor();
  }
  for (let i = meteors.length - 1; i >= 0; i--) {
    ...
    if (meteor.life > meteor.maxLife || meteor.x > width + 300 || meteor.y > height + 300) {
      meteors.splice(i, 1);
    }
  }
}
