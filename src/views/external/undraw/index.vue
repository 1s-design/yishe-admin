<template>
  <ContentWrap :plain="true">
          ...
          <div v-if="selectedClient" class="collect-panel">
            ...
            <div class="collect-section">
              <div class="collect-search__header">
                <div class="collect-section__title">插画采集 (SVG 矢量 · 可自定义主题色)</div>
                <div class="collect-search__opts">
                  <div class="collect-search__field">
                    <span class="collect-search__label">主题色</span>
                    <el-input v-model="themeColor" size="small" style="width: 100px" placeholder="#6C63FF" />
              ...
              <div class="collect-inline">
                <el-input
                  ...
                  clearable
                  placeholder="输入英文关键词搜索 undraw 开源插画（如 cat, robot, flower, banner, car）"
                  ...
                  >
                    ...
                    <div class="collect-item__thumb" @click="handlePreviewPhoto(item)">
                      <img
                        ...
                        :alt="item.title || 'undraw Illustration'"
                        loading="lazy"
                      />
                    ...
                    <div class="collect-item__info">
                      <div class="collect-item__title" :title="item.title || 'undraw 开源插画'">
                        {{ item.title || 'undraw 开源插画' }}
                      </div>
                      <div class="collect-item__meta">
                        <span v-if="item.color" class="collect-item__color-dot" :style="{ backgroundColor: item.color }" />
                        <span class="collect-item__size">Free for commercial use</span>
                      </div>
                    </div>
              ...
              <el-empty
                ...
                description="未找到相关插画，请尝试其他英文关键词"
              />
            </div>
          </div>
    ...
    <!-- 插画预览对话框 -->
    <el-dialog v-model="previewVisible" title="undraw 开源插画预览" width="700px" destroy-on-close align-center>
      <div v-if="previewItem" class="collect-preview">
        ...
        <div style="margin-top: 16px;">
          <h4>{{ previewItem.title || 'undraw 开源插画' }}</h4>
          ...
          <div style="margin-top: 8px; font-size: 13px; color: #888; display: flex; flex-direction: column; gap: 4px;">
            ...
            <span><strong>授权协议:</strong> Free for commercial and personal use (no attribution required)</span>
            <span><strong>主题色:</strong> <span :style="{ color: previewItem.color }">{{ previewItem.color }}</span></span>
...
import { getClientServiceRuntime } from '@/store/modules/clientNode';import {
  searchUndrawAndWait,
  syncUndrawToMaterialLibraryAndWait,
  refreshUndrawStatus,
  type UndrawPhoto,
  type UndrawClientVO,
  type UndrawServiceStatus,
} from '@/api/external/undraw';
import { uploadMaterialFile } from '@/api/material';
import '@/styles/external-collect.css';
import ClientSelector from '../components/ClientSelector.vue'
defineOptions({ name: 'ExternalUndraw' });

const themeColor = ref('#6C63FF');
...
const _getServiceRuntime = (c: any) => getClientServiceRuntime(c, 'undraw');;

const selectedClientId = ref('');

const clients = computed<UndrawClientVO[]>(() => {
  return onlineClients.value.map((c: any) => {
    const rawService = _getServiceRuntime(c) as UndrawServiceStatus | null;
    const runtimeService: UndrawServiceStatus | null = rawService
      ? {
          key: rawService.key,
...
const selectedService = computed<UndrawServiceStatus | null>(() => {
  return selectedClient.value?.undraw || null;
});
...
const siteStatusBadge = computed(() => {
  ...
  return isServiceAvailable.value ? 'undraw 正常' : 'undraw 异常';
});
...
const handleRefreshRuntime = async () => {
  ...
  try {
    const res = await refreshUndrawStatus(selectedClientId.value);
    if (res.success) {
...
const searchResults = ref<UndrawPhoto[]>([]);
...
const toggleSelect = (item: UndrawPhoto) => {
...
const getItemById = (id: string): UndrawPhoto | undefined => {
...
const handlePreviewPhoto = (item: UndrawPhoto) => {
  previewItem.value = item;
  previewVisible.value = true;
};

const handleSyncOne = async (item: UndrawPhoto) => {
  ...
  try {
    const result = await syncUndrawToMaterialLibraryAndWait(selectedClientId.value, {
      imageUrl: targetUrl,
      metadata: {
        title: item.title || 'undraw 开源插画',
    ...
    if (result.success) {
      ...
      await uploadMaterialFile({
        ...
        title: item.title || `undraw_${item.id}`,
        category: 'sticker',
        suffix: 'svg',
        meta: item,
      });
...
const handleBatchDownload = async () => {
  ...
  try {
    for (const id of selectedItems.value) {
      ...
      try {
        const res = await syncUndrawToMaterialLibraryAndWait(selectedClientId.value, {
          imageUrl: targetUrl,
          metadata: {
            title: item.title || 'undraw 开源插画',
            url: item.url || item.link,
...
const doSearch = async (page = 1) => {
  ...
  selectedItems.value = [];
  try {
    const result = await searchUndrawAndWait(selectedClientId.value, searchKeyword.value.trim(), {
...
const previewItem = ref<UndrawPhoto | null>(null);
...
watch(
  ...
  (newList) => {
    if (!selectedClientId.value && newList.length > 0) {
      const onlineClient = newList.find((c) => c.isOnline && c.undraw?.available);
      if (onlineClient) {
