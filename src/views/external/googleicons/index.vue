<template>
  <ContentWrap :plain="true">
          ...
          <div v-if="selectedClient" class="collect-panel">
            ...
            <div class="collect-section">
              <div class="collect-search__header">
                ...
                <div class="collect-search__opts">
                  <div class="collect-search__field">
                    ...
                    <el-select v-model="iconStyle" size="small" style="width: 110px">
                      <el-option value="outlined" label="Outlined" /><el-option value="rounded" label="Rounded" />
                      <el-option value="sharp" label="Sharp" /><el-option value="two-tone" label="Two-tone" />
                    </el-select>
                  </div>
                  <div class="collect-search__field">
              ...
              <div class="collect-inline">
                <el-input v-model="searchKeyword" clearable placeholder="输入英文关键词搜索 Google Icons（如 cat, home, settings）" @keyup.enter="handleSearch" />
                ...
                <div class="collect-list">
                  <div v-for="item in searchResults" :key="item.id" class="collect-item" :class="{ 'is-selected': selectedItems.includes(item.id) }">
                    ...
                    <div class="collect-item__info">
                      ...
                      <div class="collect-item__meta"><span class="collect-item__size">{{ item.style || iconStyle }}</span></div>
                    </div>
              ...
              <el-empty v-else-if="!searchLoading && searchKeyword" description="未找到相关图标，请尝试其他关键词" />
            </div>
          </div>
    ...
    <el-dialog v-model="previewVisible" title="Google Icon 预览" width="700px" destroy-on-close align-center>
      <div v-if="previewItem" class="collect-preview">
        ...
        <div style="margin-top: 16px;"><h4>{{ previewItem.title }}</h4>
          <div style="margin-top: 8px; font-size: 13px; color: #888; display: flex; flex-direction: column; gap: 4px;">
            <span><strong>风格:</strong> {{ previewItem.style || iconStyle }}</span>
            <span><strong>授权:</strong> Apache License 2.0</span>
            <span><strong>SVG:</strong> <a :href="previewItem.svgUrl" target="_blank" style="color: #409eff;">{{ previewItem.svgUrl }}</a></span>
          </div>
        </div>
      </div>
      <template #footer>
...
const siteStatusBadge = computed(() => !isClientOnline.value ? '不可用' : isServiceAvailable.value ? 'Google Icons 正常' : 'Google Icons 异常');
const loadClients = () => refreshClientNodes();
const handleRefreshRuntime = async () => { if (!selectedClientId.value) return; actionLoading.refreshRuntime = true; try { const r = await refreshGoogleIconsStatus(selectedClientId.value); ElMessage.success(r.success ? '已发送刷新指令' : (r.message || '刷新失败')); if (r.success) await refreshClientNodes(); } catch (e: any) { ElMessage.error(e.message || '刷新失败'); } finally { actionLoading.refreshRuntime = false; } };
const searchKeyword = ref(''); const searchLoading = ref(false); const searchResults = ref<GoogleIcon[]>([]); const searchTotal = ref(0);
...
const toggleSelect = (item: GoogleIcon) => { const idx = selectedItems.value.indexOf(item.id); if (idx > -1) selectedItems.value.splice(idx, 1); else selectedItems.value.push(item.id); };
...
const handlePreviewPhoto = (item: GoogleIcon) => { previewItem.value = item; previewVisible.value = true; };
const handleSyncOne = async (item: GoogleIcon) => {
  ...
  try { const result = await syncGoogleIconsToMaterialLibraryAndWait(selectedClientId.value, { imageUrl: targetUrl, metadata: { title: item.title, id: item.id } });
    if (result.success) { const d = result.data?.data || result.data || {}; if (!d.cosUrl) { ElMessage.error('上传 COS 失败'); return; }
      await uploadMaterialFile({ url: d.cosUrl, originUrl: item.url || targetUrl, title: item.title || `googleicon_${item.id}`, category: 'sticker', suffix: 'svg', meta: item });
...
const handleBatchDownload = async () => {
  ...
  try { for (const id of selectedItems.value) { const item = getItemById(id); if (!item) continue; const url = item.svgUrl || item.image; if (!url) continue;
    try { const r = await syncGoogleIconsToMaterialLibraryAndWait(selectedClientId.value, { imageUrl: url, metadata: { title: item.title, id: item.id } }); if (r.success) { const d = r.data?.data || r.data || {}; if (d.cosUrl) { await uploadMaterialFile({ url: d.cosUrl, originUrl: url, title: item.title, category: 'sticker', suffix: 'svg', meta: item }); ok++; } else { fail++; } } else { fail++; } } catch { fail++; } }
...
const doSearch = async (page = 1) => {
  ...
  searchLoading.value = true; selectedItems.value = []; try {
    const r = await searchGoogleIconsAndWait(selectedClientId.value, searchKeyword.value.trim(), { limit: pageSize.value, page, style: iconStyle.value });
...
const previewVisible = ref(false); const previewItem = ref<GoogleIcon | null>(null);
onMounted(() => { loadClients(); });
watch(clients, (list) => { if (!selectedClientId.value && list.length > 0) { const online = list.find((c) => c.isOnline && c['google-icons']?.available); selectedClientId.value = online?.clientId || list[0]?.clientId; } }, { immediate: true });
</script>
