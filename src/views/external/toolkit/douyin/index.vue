    ...
    <div class="toolkit-container">
      <div class="toolkit-header">
        <div class="toolkit-header__title-area">
          <span class="toolkit-title">抖音工具集</span>
      ...
      <div class="toolkit-actions-row">
        <el-button type="primary" :loading="runningAction === 'login'" :disabled="!canRun" :icon="User" @click="handleLogin">打开抖音登录</el-button>
        <el-button type="success" :icon="EditPen" :disabled="!canRun" @click="publishDialogVisible = true">发布抖音</el-button>
      </div>

      <!-- 发布弹窗 -->
      <el-dialog v-model="publishDialogVisible" title="发布抖音" fullscreen append-to-body :close-on-click-modal="false">
        <el-form :model="publishForm" label-width="70px" class="publish-form">
          <el-form-item label="视频URL">
            ...
            <div class="form-tip">抖音仅支持视频发布</div>
          </el-form-item>
...
const previewParams = computed(() => ({
  platform: 'douyin',
  video: publishForm.value.videoUrl,
...
const formatTime = (value?: string | null) => value ? formatDate(new Date(value), "YYYY-MM-DD HH:mm:ss") : "";
...
const handleLogin = async () => {
  ...
  try {
    ...
    await sendServiceCommand({
      ...
      command: { name: 'open-platform', payload: { platform: 'douyin', profileId: selectedProfileId.value, url: 'https://creator.douyin.com' } },
      mode: 'production',
    });
    ElMessage.success('已打开抖音创作者平台');
  } catch (error: any) {
...
const handlePublish = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      publishForm.value = { videoUrl: '', title: '', content: '', tags: [] };
    } else {
...
const loadStoredSession = async () => {
  if (!selectedProfileId.value) { storedPlatformSession.value = {}; return; }
  try {
    storedPlatformSession.value = asPlainObject(await getPlatformSessions({ platform: 'douyin' }));
  } catch { /* ignore */ }
};
