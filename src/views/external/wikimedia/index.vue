            ...
            <div class="collect-section">
              ...
              <div class="collect-inline">
                <el-input
                  ...
                  clearable
                  placeholder="输入关键词搜索 Wikimedia Commons（如 hitler, landscape, interior design）"
                  ...
                  >
                    ...
                    <div class="collect-item__info">
                      ...
                      <div class="collect-item__meta">
                        ...
                        <span v-if="item.license" class="collect-item__license">{{ item.license }}</span>
...
import {
  refreshWikimediaStatus,
  searchWikimediaAndWait,
  syncWikimediaToMaterialLibraryAndWait,
  type WikimediaClientVO,
  type WikimediaFile,
  type WikimediaServiceStatus,
...
defineOptions({ name: "ExternalWikimedia" });
...
const type = ref<"image">("image");
const pageSize = ref(20);
const searchResults = ref<WikimediaFile[]>([]);
const searchTotal = ref(0);
const currentPage = ref(1);
const totalPages = ref(0);
const offsetHistory = ref<(number | null)[]>([]);
...
const mapWikimediaClient = (client: any): WikimediaClientVO => ({
  clientId: client.id,
...
const selectedService = computed<WikimediaServiceStatus | null>(
  () => selectedClient.value?.wikimedia || null,
);
...
const handleSearch = async () => {
  ...
  try {
    const result = await searchWikimediaAndWait(
    ...
    offsetHistory.value[0] = result.nextOffset || null;
...
const handlePageChange = async (page: number) => {
  ...
  try {
    ...
    if (page > 1) {
      ...
      if (prevPageIndex < offsetHistory.value.length) {
        offset = offsetHistory.value[prevPageIndex];
      }
    }
    const result = await searchWikimediaAndWait(
      selectedClientId.value,
    ...
    offsetHistory.value[page - 1] = result.nextOffset || null;
  } catch (error: any) {
...
const toggleSelect = (item: WikimediaFile) => {
...
const handleSyncOne = async (item: WikimediaFile) => {
  ...
  try {
    const result = await syncWikimediaToMaterialLibraryAndWait(selectedClientId.value, {
      imageUrl: item.image,
      metadata: {
        title: item.title,
    ...
    if (result.success) {
      ...
      await uploadMaterialFile({
        ...
        name: item.title || "Wikimedia 素材",
        keywords: searchKeyword.value || "wikimedia",
        description: item.description || "",
        source: "wikimedia",
        suffix: "jpg",
        meta: item,
...
const handleRefreshRuntime = async () => {
  ...
  await trackCommand("refreshRuntime", refreshWikimediaStatus(selectedClientId.value));
};
